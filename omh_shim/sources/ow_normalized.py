...
def heart_rate_variability(sample: dict) -> dict:
    """OW ``TimeSeriesSample`` (type=heart_rate_variability) -> ``omh:heart-rate-variability:1.0``."""
    return {
...
def sleep_duration(sample: dict) -> dict:
    ...
    OMH requires a ``time_interval``, so UTC day bounds are used.
    """
    return {
...
def sleep_episode(sample: dict) -> dict:
    ...
    if (is_nap := sample.get("is_nap")) is not None:
        out["is_main_sleep"] = not bool(is_nap)
    return out
